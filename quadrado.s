...
quadrado:
	...
	li $t0, 2
	div $a2, $t0 #OBS: div por 2 eh o mesmo que shift pra direita, se o resto for ignorado
	mflo $t0
	# Checagem para ver se fica fora do bitmap.
	sub $t1, $a0, $t0 # t1 = p1x = x_centro ($a0) - L/2 ($t0)
	add $t2, $a1, $t0 # t2 = p1y = y_centro ($a1) + L/2 ($t0)
	...
	move $a3, $s1
	move $t0, $s3
	jal reta2 # reta p1 (x_cen - L/2, y_cen + L/2) -> p2 (x_cen + L/2, y_cen + L/2)
	...
	move $s1, $a3  # s1 = p3y
	move $t0, $s3
	jal reta2 # reta p2 (x_cen + L/2, y_cen + L/2) -> p3 (x_cen + L/2, y_cen - L/2)
	...
	move $s0, $a2
	move $s1, $a3
	jal reta2 # reta p3 (x_cen + L/2, y_cen - L/2) -> p4 (x_cen - L/2, y_cen - L/2)
	...
	add  $a3, $s1, $s2
	move $t0, $s3
	jal  reta2 # reta p4 (x_cen - L/2, y_cen - L/2) -> p1 (x_cen - L/2, y_cen + L/2)
